...
int main() {
    ...
    while(true){
    std::string client_id = "client-force-sensor";
    ...
    // Step 2: Read from forward kinematics data server file via GET
    std::string read_endpoint_forward_kinematics = "/read/" + read_file;
    auto res_forward_kinematics = cli.Get(read_endpoint_forward_kinematics.c_str());

    if (!res_forward_kinematics || res_forward_kinematics->status != 200) {
        std::cerr << "Failed to read from server file: " << read_file << "\n";
        if (res_forward_kinematics) {
            std::cerr << "GET request response status: " << res_forward_kinematics->status << "\n";
            std::cerr << "GET request response body: " << res_forward_kinematics->body << "\n";
        } else {
    ...
    json input_data_forward_kinematics = json::parse(res_forward_kinematics->body);
    if (!input_data_forward_kinematics.contains("values") || !input_data_forward_kinematics["values"].is_array()) {
    ...
    const auto& values = input_data_forward_kinematics["values"];
    std::cout << "Read FK values: " << values.dump(2) << "\n";
    std::vector<double> result;//= {2.0, 3.0, 4.0, 45.0,45.0,45.0};
    for (size_t i = 0; i < values.size(); ++i) {
        result.push_back(2*values[i].get<double>());
        //result += i;
    }
