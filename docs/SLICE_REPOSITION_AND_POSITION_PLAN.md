...
Reference for the geometry piece: `eetuna/cwru_data_server` (latest branch
`posix_debug`, 2025-05-04) — a single MRD receiver (like Kelvin's python-ismrmrd-server)
that DID extract position+orientation into a `FrameInfo` struct
(`mrd_server.cpp::extractFrameInfo`). That logic is what the marshal is missing.

Target branch for all work: `fix/marshal-dump-mode-switch-2026-04-19` (the rewrite,
in worktree `.worktrees/mri_data_marshal/`).
