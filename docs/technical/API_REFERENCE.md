...
- **Unified Timing:** Every event includes a `t_ms` field (integer milliseconds 
  since Unix epoch) for easy client-side latency calculation.
- **Limits:** The default HTTP body limit is 128 MiB. This can be overridden
  at startup using the `--max-body-size <bytes>` flag.
