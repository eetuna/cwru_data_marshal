...
# Fully automated MRI Data Marshal + Robot Marshal demo
# No user interaction required - runs all steps automatically
...
header() {
    ...
    echo -e "${CYAN}================================================================${NC}"
    echo -e "${CYAN}   MRI DATA MARSHAL - FULLY AUTOMATED DEMO                      ${NC}"
    echo -e "${CYAN}================================================================${NC}"
    echo ""
}
...
# ============================================================================
header
echo -e "${GREEN}MRI DATA MARSHAL - AUTOMATED COMPREHENSIVE DEMO${NC}"
echo ""
echo "This demo showcases complete MRI Data Marshal capabilities:"
echo "  • Dual-marshal architecture (MRI + Robot independent)"
echo "  • Real-time SWMR streaming to visualizer"
echo "  • Multi-protocol API support (HTTP + WebSocket)"
echo "  • Bulk file ingestion with atomic operations"
...
if [ -f "./build/viz_client" ]; then
    ...
    echo "  OpenCV window should appear and show incoming MRI frames..."
else
...
# ============================================================================
header
step_header 3 "Real-Time SWMR Streaming (192×192×10 Volumes)"

echo "Generating and streaming 5 synthetic 3D brain volumes..."
echo ""

cat > /tmp/stream_volumes.sh << 'EOF'
#!/bin/bash
MRI_HTTP=8080

for vol_num in {1..5}; do
    printf "  [Volume %d/5] Generating 192×192×10 volume... " $vol_num

    # Generate synthetic volume data (192*192*10*4 = 14.7 MB per volume)
    python3 << PYTHON
import struct
import random
...
with open(f'/tmp/volume_$vol_num.bin', 'wb') as f:
    f.write(volume_data)
PYTHON
    ...
    curl -s -X POST http://127.0.0.1:$MRI_HTTP/v1/mrd/frame \
         ...
         --data-binary @/tmp/volume_$vol_num.bin > /dev/null 2>&1

    echo "posted ✓"
    sleep 1.5  # Let visualizer catch up
done

rm -f /tmp/volume_*.bin
EOF

chmod +x /tmp/stream_volumes.sh
/tmp/stream_volumes.sh

echo ""
echo -e "${GREEN}✓ Streamed 5 volumes (192×192×10 each)${NC}"
sleep 1
...
echo ""
echo "  2. Real-Time SWMR Streaming"
echo "     • Streamed 5×(192×192×10) volumes = 19.2 MB of MRI data"
...
echo ""
echo -e "${YELLOW}For presentation:${NC}"
echo "  • Use PRESENTATION_READY.md for study materials"
echo "  • Reference PRESENTATION_STUDY_GUIDE.md for Q&A answers"
echo "  • Review ENHANCED_DEMO_GUIDE.md for technical details"
echo ""
