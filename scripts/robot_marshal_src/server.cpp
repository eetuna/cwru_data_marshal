    ...
    while (true) {
        ...
        // Wait for a write operation to be added to the queue
        {
            ...
            write_condition.wait(lock, [&]() {
                ...
                return stop_worker.load(); // Exit if the worker is stopped
            });
...
bool load_config(const std::string& config_path) {
    ...
    for (const auto& filename_json : config_json) {
        ...
        file_caches.emplace(std::piecewise_construct, std::forward_as_tuple(filename), std::forward_as_tuple(cache_capacity));
        write_queues.emplace(std::piecewise_construct, std::forward_as_tuple(filename), std::forward_as_tuple(cache_capacity));
